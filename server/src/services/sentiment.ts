...
import type { Mention, Sentiment } from '../types.js';
    ...
    try {
      const response = await chatCompletion({
        ...
        messages: [
          {
            role: 'system',
            content: 'You classify Indonesian and English public-news/social mentions by narrative risk tone for the monitored topic, not by whether the article writing style is objective. Mark adverse real-world developments as negative even in factual news copy: currency record lows, rupiah melemah, market slump, crash, crisis, scandal, allegations, protests, losses, corruption, public anger, or policy failure. Use neutral only when the event has no clear positive or negative implication. Return only JSON: {"items":[{"id":"...","sentiment":"positive|negative|neutral|mixed","confidence":0.0,"summary":"short evidence-based reason","emotions":["anger"],"intent":"complaint|praise|question|informational|null"}]}',
          },
          { role: 'user', content: JSON.stringify({ mentions: payload }) },
        ],
      });
      ...
      for (const mention of batch) {
        ...
        const finalSentiment = item.sentiment === 'neutral' && heuristic.sentiment === 'negative'
          ? 'negative'
          : item.sentiment;
